################################################################################
#
# bridge-clients
#
################################################################################

BRIDGE_CLIENTS_VERSION = main
BRIDGE_CLIENTS_SITE_METHOD = git
BRIDGE_CLIENTS_SITE = git@git.integraal.info:integraal/clients.git
BRIDGE_CLIENTS_INSTALL_STAGING = YES
BRIDGE_CLIENTS_INSTALL_TARGET = YES

BRIDGE_CLIENTS_DEPENDENCIES = bridge bridge-contracts

BRIDGE_CLIENTS_SUBDIR = src

ifeq ($(BR2_PACKAGE_BRIDGE_VIRTUALINPUT),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DVIRTUALINPUT=ON
endif

ifeq ($(BR2_PACKAGE_BRIDGE_COMPOSITORCLIENT),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DCOMPOSITORCLIENT=ON
    ifeq ($(BR2_PACKAGE_MESA3D),y)
        BRIDGE_CLIENTS_CONF_OPTS += -DPLUGIN_COMPOSITOR_IMPLEMENTATION=Mesa
        BRIDGE_CLIENTS_CONF_OPTS += -DCOMPOSITORBUFFER=ON
        BRIDGE_CLIENTS_DEPENDENCIES += mesa3d
    else ifeq ($(BR2_PACKAGE_WESTEROS),y)
        BRIDGE_CLIENTS_CONF_OPTS += -DPLUGIN_COMPOSITOR_IMPLEMENTATION=Wayland
        BRIDGE_CLIENTS_CONF_OPTS += -DPLUGIN_COMPOSITOR_SUB_IMPLEMENTATION=Westeros
        BRIDGE_CLIENTS_DEPENDENCIES += westeros
    else ifeq ($(BR2_PACKAGE_WESTON),y)
        BRIDGE_CLIENTS_CONF_OPTS += -DPLUGIN_COMPOSITOR_IMPLEMENTATION=Wayland
        BRIDGE_CLIENTS_CONF_OPTS += -DPLUGIN_COMPOSITOR_SUB_IMPLEMENTATION=Weston
        BRIDGE_CLIENTS_DEPENDENCIES += weston
    else ifeq ($(BR2_PACKAGE_HAS_NEXUS),y)
        BRIDGE_CLIENTS_DEPENDENCIES += bcm-refsw
        BRIDGE_CLIENTS_CONF_OPTS += -DPLUGIN_COMPOSITOR_IMPLEMENTATION=Nexus
    else ifeq  ($(BR2_PACKAGE_RPI_USERLAND),y)
        BRIDGE_CLIENTS_DEPENDENCIES += rpi-userland
        BRIDGE_CLIENTS_CONF_OPTS += -DPLUGIN_COMPOSITOR_IMPLEMENTATION=RPI
    else
        $(error Missing a compositor implemtation, please provide one or disable PLUGIN_COMPOSITOR)
    endif
endif

ifeq ($(BR2_PACKAGE_BRIDGE_OPENCDM),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DCDMI_ADAPTER_IMPLEMENTATION=gstreamer
    BRIDGE_CLIENTS_CONF_OPTS += -DCDMI=ON
    BRIDGE_CLIENTS_DEPENDENCIES += gstreamer1
endif

ifeq ($(BR2_PACKAGE_BRIDGE_PROVISIONPROXY),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DPROVISIONPROXY=ON
    BRIDGE_CLIENTS_DEPENDENCIES += libprovision
endif

ifeq ($(BR2_PACKAGE_BRIDGE_PLAYERINFO),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DPLAYERINFO=ON
    BRIDGE_CLIENTS_DEPENDENCIES += gstreamer1
endif

ifeq ($(BR2_PACKAGE_BRIDGE_DEVICEINFO),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DDEVICEINFO=ON
endif

ifeq ($(BR2_PACKAGE_BRIDGE_DISPLAYINFO),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DDISPLAYINFO=ON
endif

ifeq ($(BR2_PACKAGE_PLUGIN_BUTLER),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DBUTLERCLIENT=ON
endif

ifeq ($(BR2_PACKAGE_BRIDGE_CRYPTOGRAPHY),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DCRYPTOGRAPHY=ON
    BRIDGE_CLIENTS_CONF_OPTS += -DINCLUDE_SOFTWARE_CRYPTOGRAPHY_LIBRARY=ON
    BRIDGE_CLIENTS_CONF_OPTS += -DCRYPTOGRAPHY_IMPLEMENTATION=OpenSSL
endif

ifeq ($(BR2_PACKAGE_BRIDGE_GRAPHICS_BUFFER),y)
    BRIDGE_CLIENTS_CONF_OPTS += -DGRAPHICSBUFFER=ON
endif



$(eval $(cmake-package))
